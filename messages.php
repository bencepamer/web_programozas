...
// Törlés kezelése AJAX kérésként, ha POST kérés érkezik
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'delete_message') {
    ...
    try {
        ...
        // Hiba kezelése, ha nem sikerült az üzenet törlése
        echo json_encode(['status' => 'error', 'message' => 'Hiba történt az üzenet törlése közben: ' . $e->getMessage()]);
        exit();
    }
}

// Bejövő üzenetek lekérdezése a felhasználónak
try {
    ...
    // Hiba kezelése, ha nem sikerült lekérni az üzeneteket
    $_SESSION['error_message'] = "Hiba történt az üzenetek lekérése közben: " . $e->getMessage();
    header("Location: index.php"); // Vagy más kezdőlapra irányítás
    exit();
}
?>
